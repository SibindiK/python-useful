...
def show_result(result, player_cards, dealer_cards, cards_deck):
...
def play_black_jack(cards_deck, player_cards, dealer_cards):
    '''play the black jack game'''
    player_cards.append(get_card(cards_deck))
    player_cards.append(get_card(cards_deck))
    print(
        f"Your cards are [{player_cards[0]}] [{get_card(player_cards[1])}] ")
    ...
    while another_card:
        ...
        if draw_again == 'd':
            ...
            if player_total > dealer_total and player_total < 22:
                show_result("WIN", player_cards, dealer_cards, cards_deck)
            elif player_total < dealer_total or player_total > 21:
                show_result("LOSE", player_cards, dealer_cards, cards_deck)
            elif player_total == dealer_total and player_total < 21:
                show_result("DRAW", player_cards, dealer_cards, cards_deck)
...
def main():
    ...
    while another_game:
        ...
        if play_game == 'y':
            player_cards = []
            dealer_cards = []
            play_black_jack(deck, player_cards, dealer_cards)
        elif play_game == 'n':
            another_game = False
